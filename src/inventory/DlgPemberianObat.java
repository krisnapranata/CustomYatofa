...
public class DlgPemberianObat extends javax.swing.JDialog {
    ...
    private void initComponents() {
        ...
        BtnPrint.setPreferredSize(new java.awt.Dimension(70, 30));
        ...
        BtnKirimBerkas.setIcon(new javax.swing.ImageIcon(getClass().getResource("/picture/uparrow2_m.png"))); // NOI18N
        BtnKirimBerkas.setMnemonic('T');
        BtnKirimBerkas.setText("Kirim Berkas");
        BtnKirimBerkas.setToolTipText("Alt+T");
        BtnKirimBerkas.setHorizontalAlignment(javax.swing.SwingConstants.LEFT);
        BtnKirimBerkas.setName("BtnKirimBerkas"); // NOI18N
        BtnKirimBerkas.setPreferredSize(new java.awt.Dimension(120, 30));
        ...
        panelGlass8.add(BtnKirimBerkas);
  ...
  isForm();                
    ...
    private void BtnKirimBerkasActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnKirimBerkasActionPerformed
        ...
        if(tabModePO.getRowCount()==0){
            ...
            tgl=" detail_pemberian_obat.tgl_perawatan between '"+Valid.SetTgl(DTPCari1.getSelectedItem()+"")+"' and '"+Valid.SetTgl(DTPCari2.getSelectedItem()+"")+"' "+pas;        
//            System.out.println(TCariPasien.getText());
            
            customsimrs.SimpanBerkasKeTabelBerkas(
                    TNoRw.getText(),                     
                    koneksiDB.KODEEVALUASIOBAT(), 
                    TPasien.getText(),  
                    TNoRM.getText(),
                    "EVALUASI_OBAT.pdf");
            
            System.out.println(TNoRw.getText());
            customsimrs.MyReportQryPDFBaru("rptBrObt.jasper","report","::[ Rekam Data Pemberian Obat (UMUM) ]::",
                   "select detail_pemberian_obat.tgl_perawatan,detail_pemberian_obat.jam,"+
                   "detail_pemberian_obat.no_rawat,reg_periksa.no_rkm_medis,pasien.nm_pasien,"+
                   "detail_pemberian_obat.kode_brng,databarang.nama_brng,detail_pemberian_obat.embalase,detail_pemberian_obat.tuslah,"+
                   "detail_pemberian_obat.jml,detail_pemberian_obat.biaya_obat,detail_pemberian_obat.total,detail_pemberian_obat.h_beli,"+
                   ...
                   "pasien.nm_pasien like '%"+TCari.getText().trim()+"%' or detail_pemberian_obat.kode_brng like '%"+TCari.getText().trim()+"%' or "+
                   "databarang.nama_brng like '%"+TCari.getText().trim()+"%' or detail_pemberian_obat.no_faktur like '%"+TCari.getText().trim()+"%' or "+
                   "detail_pemberian_obat.no_batch like '%"+TCari.getText().trim()+"%') ")+" order by detail_pemberian_obat.tgl_perawatan",
                   TPasien.getText(),  
                   TNoRM.getText(),
                   "EVALUASI_OBAT.pdf", param);
