...
public final class RMDataResumePasien extends javax.swing.JDialog {
    ...
    private void initComponents() {
        ...
        MnKirimBerkas.setActionCommand("Kirim Berkas");
        MnKirimBerkas.setLabel("Kirim Berkas");
        MnKirimBerkas.setName("MnKirimBerkas"); // NOI18N
        MnKirimBerkas.setPreferredSize(new java.awt.Dimension(220, 26));
    ...
    private void BtnSimpanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnSimpanActionPerformed
        if(TNoRw.getText().equals("")||TNoRM.getText().equals("")||TPasien.getText().equals("")){
            ...
            if(Sequel.menyimpantf("resume_pasien","?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?","No.Rawat",26,new String[]{
                ...
                /*
                // Memanggil Method Otomatis Kirim Berkas
                */
                if(koneksiDB.WORKINGDIRSFTPUPLOADBERKAS().equals("yes")){
                    OtomatisKirimBerkasPdfDariBtnSimpan(evt);
                }
//                OtomatisKirimBerkasPdfDariBtnSimpan(evt);
//                OtomatisKirimSPBKDariBtnSimpan(evt);
    ...
    /*
    // MENAMBAHKAN METHOD KIRIM BERKAS OTOMATIS STELAH KLIK SIMPAN RESUME AKAN
    // DIPAKAI OLEH METHOD BtnSimpanActionPerformed
    */
    private void OtomatisKirimBerkasPdfDariBtnSimpan(java.awt.event.ActionEvent evt) {
        Map<String, Object> param = new HashMap<>();
            ...
            //tambahan baris baru
            String resume = "_resume_ralan.pdf";
            ...
            if(Sequel.cariIsi("select reg_periksa.status_lanjut from reg_periksa where reg_periksa.no_rawat=?",TNoRw.getText()).equals("Ralan")){
                ...
                tanggal=Sequel.cariIsi("select DATE_FORMAT(tgl_registrasi, '%d-%m-%Y') from reg_periksa where no_rawat=?",TNoRw.getText());
                param.put("tanggalkeluar",tanggal);
            }else{
                param.put("ruang",Sequel.cariIsi("select nm_bangsal from bangsal inner join kamar inner join kamar_inap on bangsal.kd_bangsal=kamar.kd_bangsal and kamar_inap.kd_kamar=kamar.kd_kamar where no_rawat=? order by tgl_masuk desc limit 1 ",TNoRw.getText()));
                tanggal=Sequel.cariIsi("select DATE_FORMAT(tgl_keluar, '%d-%m-%Y') from kamar_inap where no_rawat=? order by tgl_keluar desc limit 1 ",TNoRw.getText());
                param.put("tanggalkeluar",tanggal);
            }
            ...
            Sequel.menyimpan3("berkas_digital_perawatan", "?,?,?","No.Rawat",3,new String[]{
                TNoRw.getText(),"RESUME","pages/upload/"+namapasien+"_"+TNoRM.getText()+resume
            });
            
            Valid.MyReportPDFResumeRalan("rptLaporanResume.jasper","report","::[ Laporan Resume Pasien ]::",param);
    }
    ...
    private void BtnEditActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnEditActionPerformed
        if(TNoRw.getText().equals("")||TNoRM.getText().equals("")||TPasien.getText().equals("")){
            ...
            if(tbObat.getSelectedRow()>-1){
                if(akses.getkode().equals("Admin Utama")){
                    ganti();
//                    OtomatisKirimBerkasPdfDariBtnGanti(evt);
                }else{
                    if(KodeDokter.getText().equals(tbObat.getValueAt(tbObat.getSelectedRow(),4).toString())){
                        ganti();
//                        OtomatisKirimBerkasPdfDariBtnGanti(evt);
    ...
    private void OtomatisKirimSPBKActionPerformed(java.awt.event.ActionEvent evt) {                                       
//        if(TPasien.getText().trim().equals("")){
//            JOptionPane.showMessageDialog(null,"Maaf, Silahkan anda pilih dulu pasien...!!!");
//        }else{
//            this.setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
            ...
            String resume = "_resume_ralan.pdf";
            param.put("resume", resume);
            String nm_pasien = tbObat.getValueAt(tbObat.getSelectedRow(),3).toString();
            String namapasien = nm_pasien.replace(" ", "_");
            param.put("nmpasien",namapasien);
            param.put("norm",tbObat.getValueAt(tbObat.getSelectedRow(),2).toString());
            param.put("host", koneksiDB.HOSTHYBRIDWEB());
            param.put("usersftp", koneksiDB.USERSFTPUPLOADBERKAS());
            param.put("passwordsftp", koneksiDB.PASSWORDSFTPUPLOADBERKAS());
            param.put("portsftp", koneksiDB.PORTSFTPUPLOADBERKAS());
            ...
            param.put("logo",Sequel.cariGambar("select setting.logo from setting"));
            Sequel.menyimpan3("berkas_digital_perawatan", "?,?,?","No.Rawat",3,new String[]{
                TNoRw.getText(),"RESUME","pages/upload/"+namapasien+"_"+tbObat.getValueAt(tbObat.getSelectedRow(),2).toString()+resume
            });
            
//            Valid.MyReportqrypdfSPBKBaru("rptSBPK.jasper","report","::[ Surat Bukti Pelayanan Kesehatan ]::",
            Valid.MyReportqrypdfSPBKBaru("rptSBPK.jasper","report","::[ Surat Bukti Pelayanan Kesehatan ]::",
                "select reg_periksa.no_reg,reg_periksa.no_rawat,reg_periksa.tgl_registrasi,"+
                "reg_periksa.jam_reg, reg_periksa.kd_dokter,dokter.nm_dokter,reg_periksa.no_rkm_medis,pasien.nm_pasien,pasien.jk,concat(reg_periksa.umurdaftar,' ',reg_periksa.sttsumur)as umur,poliklinik.nm_poli," +
                "reg_periksa.p_jawab,reg_periksa.almt_pj,reg_periksa.hubunganpj,reg_periksa.biaya_reg," +
...
//        }
    ...
    private void OtomatisKirimSPBKDariBtnGanti() {                                       
//        if(TPasien.getText().trim().equals("")){
//            JOptionPane.showMessageDialog(null,"Maaf, Silahkan anda pilih dulu pasien...!!!");
//        }else{
//            this.setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
...
//        }
    ...
    private void OtomatisKirimSPBKDariBtnSimpan(java.awt.event.ActionEvent evt) {                                       
//        if(TPasien.getText().trim().equals("")){
//            JOptionPane.showMessageDialog(null,"Maaf, Silahkan anda pilih dulu pasien...!!!");
//        }else{
//            this.setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
...
//        }
    ...
    /*
    // MENAMBAHKAN METHOD UNTUK UPLOAD OTO RESUME RALAN
    */
...
//                TNoRw.getText(),"RESUME","pages/upload/"+namapasien+"_"+tbObat.getValueAt(tbObat.getSelectedRow(),2).toString()+resume
//            });
            customsimrs.SimpanBerkasKeTabelBerkas(TNoRw.getText(), koneksiDB.KODERESUME(), tbObat.getValueAt(tbObat.getSelectedRow(),3).toString(), tbObat.getValueAt(tbObat.getSelectedRow(),2).toString(), "resume_ralan.pdf");
            customsimrs.MyReportPDFBaru("rptLaporanResume.jasper","report","::[ Laporan Resume Pasien ]::", tbObat.getValueAt(tbObat.getSelectedRow(),3).toString(), tbObat.getValueAt(tbObat.getSelectedRow(),2).toString(), "resume_ralan.pdf", param);
//            Valid.MyReportPDFResumeRalan("rptLaporanResume.jasper","report","::[ Laporan Resume Pasien ]::",param);
        
        }
//        OtomatisKirimSPBKActionPerformed(evt);
    ...
    */
    ...
    private void ganti() {
        if(Sequel.mengedittf("resume_pasien","no_rawat=?","no_rawat=?,kd_dokter=?,keluhan_utama=?,jalannya_penyakit=?,pemeriksaan_penunjang=?,hasil_laborat=?,diagnosa_utama=?,kd_diagnosa_utama=?,diagnosa_sekunder=?,kd_diagnosa_sekunder=?,diagnosa_sekunder2=?,kd_diagnosa_sekunder2=?,diagnosa_sekunder3=?,kd_diagnosa_sekunder3=?,diagnosa_sekunder4=?,kd_diagnosa_sekunder4=?,prosedur_utama=?,kd_prosedur_utama=?,prosedur_sekunder=?,kd_prosedur_sekunder=?,prosedur_sekunder2=?,kd_prosedur_sekunder2=?,prosedur_sekunder3=?,kd_prosedur_sekunder3=?,kondisi_pulang=?,obat_pulang=?",27,new String[]{
            ...
            if(koneksiDB.UPLOADBERKASOTOMATIS().equals("yes")){
                OtomatisKirimBerkasPdfDariBtnGanti();
            }
//            OtomatisKirimBerkasPdfDariBtnGanti();
//            OtomatisKirimSPBKDariBtnGanti();
            emptTeks();
        }
    }
    
    private void OtomatisKirimBerkasPdfDariBtnGanti() {
        Map<String, Object> param = new HashMap<>();
        param.put("namars", akses.getnamars());
        param.put("alamatrs", akses.getalamatrs());
        param.put("kotars", akses.getkabupatenrs());
