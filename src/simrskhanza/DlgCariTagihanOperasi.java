    ...
    private void MnKirimBerkasActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_MnKirimBerkasActionPerformed
        if (tbDokter.getSelectedRow() > -1) {
            if (!tbDokter.getValueAt(tbDokter.getSelectedRow(), 1).toString().equals("")) {
                ...
                customsimrs.SimpanBerkasKeTabelBerkasOperasi(tbDokter.getValueAt(tbDokter.getSelectedRow(), 1).toString(), koneksiDB.KODELAPORANOPERASI(), tbDokter.getValueAt(tbDokter.getSelectedRow(), 2).toString(), "laporan_operasi.pdf");
                customsimrs.MyReportPDFLaporanOperasiBaru("rptLaporanOperasi.jasper", "report", "::[ Laporan Operasi ]::",tbDokter.getValueAt(tbDokter.getSelectedRow(), 2).toString(), "laporan_operasi.pdf", param);
//                Valid.MyReportPDFLaporanOperasiBaru("rptLaporanOperasi.jasper", "report", "::[ Laporan Operasi ]::", param);
            } else {
                JOptionPane.showMessageDialog(rootPane, "Silahkan pilih data, klik pada No.Rawat ..!!");
            }
        }
